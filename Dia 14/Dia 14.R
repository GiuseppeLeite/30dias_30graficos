...
stopifnot(identical(names(gene_exp_data_ordered), sample_annot$SampleName))
...
write.table(alive_dead_DEGs_filtered, file = "alive_dead_DEGs_filtered.txt", row.names = FALSE)
...
write.table(filtered_gene_exp_data, file = "filtered_gene_exp_data.txt", sep = "\t")
...
pheatmap(
  ...
  cluster_cols = TRUE,    # Clustering de colunas
